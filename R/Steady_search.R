...
#'
#' This function generates a random ID, constructs a path for the output, and
#' saves a file in the specified location.
#'
#' @param n_species Number of species involved in the simulation.
#' @param params List including:
#' \itemize{
#'   \item Interaction matrix: If NULL, \code{runif(n = n_species, min = 0, max = 1)} is used.
#'   \item Growth rates: Numeric vector representing the growth rates of simulated species.
#'   \item Population: Numeric vector representing the initial abundances of simulated species.
#' }
#' @param times Numeric: Number of generations to simulate.
#' @param norm Logical: Whether the time series should be returned with the abundances as proportions (\code{norm = TRUE}) or
#' the raw counts (\code{norm = FALSE}, default).
#'
