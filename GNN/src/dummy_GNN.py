    ...
    for i, iter in enumerate(tqdm(range(epochs), desc="Training")):
        ...
        for path in batched_paths:
            ...
            for data in data_list:
                #----------------------
                # Section: Load only the dummy data (last feature)
                # data = data_list[2]
                data.x = data.x[:, -1]
...
result_path = f'{results_dir}/{os.path.basename(data_dir)}/Raw-Dummy'
os.makedirs(result_path, exist_ok=True)
#-------------------------------
# Save max indexes
idx_max_true.to_feather(f'{result_path}/max_indexes_true.feather')
idx_max_pred.to_feather(f'{result_path}/max_indexes_pred.feather')
#-------------------------------
# Save metrics
df_true.to_feather(f'{result_path}/Metrics_true.feather')
df_pred.to_feather(f'{result_path}/Metrics_predicted.feather')
#-------------------------------
# Save loss history
np.save(f'{result_path}/Dummy_loss.npy', np.array(loss_history))
...
plt.ylabel("Loss")
plt.grid(True)
plt.savefig(f'{result_path}/loss_history_plot.png')
