...
solve_gLV <- function(times, params) {
  ...
  # Get solution
  results <- tryCatch(
    R.utils::withTimeout(deSolve::ode(y = params$x0, # Variable to evaluate
  ...
  # Set abundances to zero if the value is less than 10^-8 to calculate derivative. Given that differential equation
  results[ results < 10^-8 ] <- 0
