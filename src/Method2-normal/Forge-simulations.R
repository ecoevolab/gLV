...
workdir <-  "/mnt/atgc-d3/sur/users/mrivera/glv-research/Results/ExpM279-D06M03"
...
ode_dir <- file.path(workdir, "Simulate_ODE")
...
parallel.sims <- function(index, path_ODE) {
  ...
  # Generate parameters
  params <- regenerate_Dnormal(index)
...
save.path <- file.path(workdir, "Nas-counting.tsv")
...
source_path <- file.path(workdir, "Simulate_ODE")
target_path <- file.path(workdir, "Unified")
