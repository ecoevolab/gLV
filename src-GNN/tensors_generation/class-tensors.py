...
def load_single_data(row, interactions_dir, targets_dir, topologies_dir):
    ...
    #-------------------------------
    # Section: Generate target features
    tgt_path = os.path.join(targets_dir, f"ExtSummary_{id}.feather")
    rel_pops =  feather.read_table(tgt_path, columns=['rel_pop_initial']).to_pandas()
    # Filter extinct species
    alive_vector = rel_pops > 1e-06                 # vector
    ...
    network_stats = network_stats.drop(columns=["total_degree"])                # Remove total degree
    x_tensor = torch.from_numpy(network_stats.to_numpy(dtype=np.float32))       # Convert tensor
...
row = data.iloc[0]
x = load_single_data(row, interactions_dir, targets_dir, topologies_dir)
...
#-----------------------
# Run parallelization
tensors_path = '/mnt/data/sur/users/mrivera/Cuda-tensors/Boosted_filter-classes'
batching(df = data,
