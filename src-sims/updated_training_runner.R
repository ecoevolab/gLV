...
#----------------------------------------------
tictoc::tic("Section 1: Generate parameters and create directories")
start = Sys.time()
...
while (experiment_id %in% basename(exist)) {
  experiment_id = substr(ids::random_id(),1, 12) # Regenerate if ID already exists
}     
...
wrapper <- function(index, df_params, ext_threshold, experiment_dir) {
  ...
  summary_exts_path <- make_path(experiment_dir, 'ExtSummaries','ExtSummary_', sim_id)   
  # Save files
  ...
  #-----------------------------
  # Section: Simulate survivor nodes extinctions with impact in full community
  ...
  arrow::write_feather(x = as.data.frame(summary_full), sink = summary_exts_path)   
  #-----------------------------
  cat(">> Simulation ", sim_id, " completed.\n")  
...
summary_text <- paste0(
  "Experiment summary\n",
  "=========================\n",
  "Method used: Proportions of zeros and interactions. \n",
  "Method used for extinction: Extinctions were performed at only survival nodes, impact was measured at full community level. \n",
  ...
  'Elapsed time: ', round(elapsed / 60, 2), ' minutes \n'
)
