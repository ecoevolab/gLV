# =============================================================================
# Graph generation for controls simulation, boosting column method.
# =============================================================================
...
p_noint <- runif(1, min = 0, max = 0.3)
p_neg   <- 1

# Interaction counts
n_offdiag <- n_species^2 - n_species                # off-diagonal elements 
...
# Interaction matrix
M        <- matrix(NA, n_species, n_species, dimnames = list(paste0("S", 1:n_species), paste0("S", 1:n_species)))
diag(M)  <- -0.5
M[!diag(n_species)] <- sample( c(rep(0, n_noint),-runif(n_neg, min = 0, max = 1), runif(n_pos, min = 0, max = 1)) ) 
M[, k] <- M[, k] * 10
M  <- round(M, 5)

edges_to_highlight <- data.frame(
    from = paste0("S", k),
    to   =  paste0("S", 1:n_species)
)
...
graph_eg <- function(mat, highlight_node = NULL, highlight_color = "orange", highlight_edges = NULL) {
    ...
    ggraph::ggraph(g, layout = "manual", x = layout$x, y = layout$y) +
        ...
        labs(
            ...
            subtitle = "Método de incremento de columna"
        ) +
        theme(
        ...
        annotate(
            "richtext",
            x = Inf, y = Inf,
           label = "<span style='color:darkred'>&#9632;</span> Interacción * 10 <br>
         <span style='color:steelblue'>&#9632;</span> Interacción: 0 or U(-1, 0)",
...
print(peg)
ggsave(paste0(save_dir, "/boosted_graph.png"), plot = peg, width = 8, height = 6, dpi = 300)
