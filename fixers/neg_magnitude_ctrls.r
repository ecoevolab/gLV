...
id <- substr(ids::uuid(1, drop_hyphens = TRUE, use_time = TRUE), start=1, stop=12)            
experiment_id <- paste0("Negative_magnitude_", id) # Experiment-ID
...
wrapper <- function(index, df_params, experiment_dir) {
  #-----------------------------
  # Section: Generate parameters and run simulation
  row = df_params[index, ] 
  sim_id = row$id
  # Generate parameters
  params <- gen_neg_mag_params(row)                                # Generate-parameters
  ...
  # Extinctions of all species in the community
  ...
  return(list(id = sim_id, na_ct = na_count, tts_output = out_stability_time, tts_ext = extinction_stability_time))
}
...
summary_text <- paste0(
  "Experiment summary\n",
  "=========================\n",
  "Method used: Negative magnitudes \n",
  "Time for solver: 1000 \n",
  "Number of cores: ", ncore, "\n",
  "Extinction method: All species extinctions \n",
